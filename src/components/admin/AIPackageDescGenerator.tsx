...
- (선택) 셋째 문단 — **"상품구성"** 정보: 촬영지·장소, 원본/보정 컷 수, 촬영 시간, 가격·인원 추가 옵션, 의상, 포함 사항 등 **패키지 스펙**. **스크린샷·이미지에 명시된 숫자·항목만** 그대로 옮겨 적기 (없는 정보는 절대 추측하지 말기). **이런 스펙 정보는 Tips가 아니라 반드시 여기(details의 상품구성)에 와야 합니다.**
- **상품구성 포맷 (반드시 이 형식)**: 한 줄 한 항목씩 \`1. 2. 3. ...\` 번호 매김. 줄바꿈은 \\n. 각 줄은 한 가지 스펙만, 간결한 한 문장. 어미는 casual하게("~제공", "~진행돼요", "~추가돼요"). 줄글 단락 금지.
- 좋은 예 (이 포맷 그대로):
  \`\`\`
  1. 장소: 1곳
  2. 원본 JPG 약 500장 전체 제공
  3. 보정본은 작가 셀렉 5장 + 고객 셀렉 10장으로 총 15장 제공
  4. 가격은 초상권 동의 시 100,000원, 미동의 시 120,000원이고, 2인 기준이라 인원 추가 시 1인당 50,000원이 추가돼요.
  5. 의상은 환복 없이 1벌로 진행돼요.
  \`\`\`
- 스크린샷에 정보가 4개 이하면 4개로, 6개 이상이면 6개로 쓰면 됨. 항목 수는 강제 아님 (스크린샷 기반).
- 길지 않게, 카드 본문처럼 부담 없이
- 좋은 예시 (만삭 스냅):
...
- **절대 금지 (Tips에 들어가면 안 됨)**: 구체적인 촬영지 이름(협재해변, 새별오름, 삼나무숲 등), 원본/보정 컷 수, 촬영 시간(분/시간), 가격·추가 옵션 — **이런 패키지 스펙 정보는 모두 details의 "상품구성" 문단에 들어가야 함**. Tips는 유저가 준비할 행동(시기, 의상, 소품, 날씨 주의)만.
- 좋은 예:
